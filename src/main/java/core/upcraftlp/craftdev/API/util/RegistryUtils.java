...
public class RegistryUtils {
	...
	public static <T extends IForgeRegistryEntry<T>> void createRegistryEntries(Class<T> type,
			RegistryEvent.Register<T> event, Class clazz, String modid, CreativeTabs tab) {
		...
		for (Field f : clazz.getDeclaredFields()) {
			if (IForgeRegistryEntry.class.isAssignableFrom(f.getType())) {
				try {
					...
					// event.getRegistry().register(entry); //FIXME why does
					// this not work?
				...
				if (!Modifier.isFinal(f.getModifiers()))
					CraftDevCore.getLogger().println(
							clazz.getName() + ":" + f.getName() + " has no final modifier! Pleaes change this!");
			}
		}
		...
		if (item.getHasSubtypes()) {
			...
			for (ItemStack stack : items) {
				...
				ModelLoader.setCustomModelResourceLocation(item, meta,
						new ModelResourceLocation(item.getRegistryName() + "_" + meta, "inventory"));
			}
		} else {
